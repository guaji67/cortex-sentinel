...
| 通道 | `channelStatus` + 本机引擎计数 | 三卡 Codex / CodeBuddy / Grok。`channel-status.json` 落后于最新派工线状态文件或放过 10 分钟，哨兵按 cortex `scripts/channel_status.files` 导出 `channel_status.py --logs-dir <监视目录>` 重算，60 秒节流、失败退避 10 分钟；判据仍只在 cortex | `ChannelSectionPresentationTests`、`CortexChannelStatusRefreshTests` |
...
组与组之间不许再放无含义的小圆点分隔符（已随 0.1.17 删除，别加回来）。改名：点行名编辑，回车或点任意空白处提交，Esc 取消。详情卡：标签/数值/重置时间三列，重置时间越近越醒目（≥60% 流逝黄、≥85% 红）。所有详情卡（GLM / CC 共用一套布局）标签列 76、卡宽 312：执行者短名「M1Max ZCode」75pt、58 列放不下（量宽实锤），列宽一起抬；标签加宽挤掉的余量用卡宽补回（值+备注最长组合约 184、渲染再放宽也不截）。两个套餐卡的执行者短名可能重名，靠卡片标题（套餐名）区分，标签列不加字。GLM 窗口数值不带「积分」字样——原卡宽 296 里带着它重置时间必截（量宽实锤），CC 卡口径本就没带。「免费时段」标签已带词，值里 cortex 文案若含「免费时段 / 现在是」要去掉重复；套餐冷却中时「派工」行写「冷却到 HH:MM，暂不派工」，跟行上冷却文案一致；同一时刻既冷却又有 cortex 给的拦人理由时，两边都是真话，行和卡先显示冷却，是有意的。卡片必须盖在余额区所有行上面：行级 zIndex 只在分支内兄弟间排序，GLM/CC 两分支的卡显示状态经 `BalanceCardBranchKey` 上报、分支容器按此抬 zIndex。
...
套餐行的第三列（原现金格）换在跑/冷却文案：冷却中写「冷却到 HH:MM」（放不下自动换「冷却 HH:MM」，不出图截断为准）、在跑写「在跑 n/上限」、读不到写「在跑 —」；这列不画横条（横条一律是时间流逝，这列不是时间）、不加 `.help`（会和详情卡双弹）。「在跑 n/上限」数的是占着的位子：cortex 把 running、queued、dispatched、waiting_local_directory 四种都算——排队的同样占派工名额，不改字眼。套餐数据由 cortex 仓 `scripts/glm_plan_status.py --json` 提供（按 origin/main 清单导出缓存后运行），失败时最近一次成功的整份留在内存当套餐身份（指纹/名/上限，不落盘，下次成功整份换新）：30 分钟内数照用，详情卡末尾加「派工状态：HH:MM 读到的，这次没读到（原因）」；超过 30 分钟数判过时——行名仍用套餐名，第三列固定「在跑 —」不显示冷却，状态点只看订阅窗口，详情卡只留在跑「— / 上限 n」、现金余额、「派工状态：没读到（原因）」三行，执行者/派工/免费时段不显示；开 App 以来一次都没成功过则行照旧，`--dump-state` 现场跑一轮打一行排查。指纹/套餐 id/错误 code 不进界面。守卫：`CortexPlanStatusTests.testThirdColumnCooldownRunningAndUnknown`、`testFreshFailureKeepsNumbersAndAddsNote`、`testStaleFailureShowsIdentityOnly`、`testNeverSucceededKeepsRowsUntouched`、`testGitSubcommandsAreReadOnlyWhitelist`。

同账号附加钥匙归并（面板余额区唯一藏行的地方）：cortex 套餐清单多带一个 `also_key_sha12` 字段（同账号其他钥匙的指纹前 12 位），余额区正常只显示套餐行，被归并的附加钥匙行不单独成行；归并只认 cortex 登记的附加指纹，不是套餐的行一字不改。主钥匙匹配优先——同一指纹既是 A 套餐主钥匙又被 B 套餐列为附加，按 A 的套餐行显示；套餐主钥匙行不在列表里时（本机没认出主钥匙）附加行不藏，免得整个套餐从面板上消失。并完还要看得见：那把代理钥匙可能单独出错（钥匙被换、代理那条链路坏），详情卡在追加行之后每把附加钥匙一行，标签用那行原来的行名（用户改过名用改过的，没改过用 displayTitle），状况为空写「同账号，额度共用」，出错/过时/读不到额度/跟套餐读数对不上写对应人话（重置时刻差 60 秒以内、百分比差 2 个点以内算同一份——两把钥匙的用量不在同一瞬间读）；现金主钥匙行没有就用附加行的数（同账号现金是同一份）。指纹、套餐 id、错误 code 不进卡片。拖拽排序只排可见行，登记顺序里被藏的钥匙不占位（排序表对不在场的键保持原相对顺序，不会错位）。守卫：`CortexPlanStatusTests.testAlsoKeySHA12DecodesListAndDefaultsToEmpty`、`testMergeTargetFindsPlanByAdditionalFingerprint`、`testVisibleRowsHideAdditionalKeyRow`、`testVisibleRowsKeepAdditionalWhenPrimaryMissing`、`testVisibleRowsPrimaryMatchBeatsAdditionalListing`、`testVisibleRowsKeepUnaffiliatedRows`、`testAdditionalKeyStatusPhrases`、`testAdditionalKeyCardLinesCarryPlainPhrasesOnly`、`testPlanCashFallsBackToAdditionalRow`。
