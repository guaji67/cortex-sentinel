...
两条路装出来的是同一个 app，开机自启都会配好，区别只在装的时候要不要本机有 Swift。

## 装法一：拖进「应用程序」

到 [Releases](../../releases) 下载编好的安装盘（`.dmg`），打开后一侧是应用、一侧是「应用程序」快捷方式，把应用拖过去。

Releases 里的包是 Developer ID 签名并过了 Apple 公证的，正常双击就能打开，不会被系统拦。只有你自己本地 ad-hoc 签名构建出来的包（见「自己构建 app」）才需要走下面的「被系统拦住」流程。

### 本地构建的包第一次打开会被系统拦住

这个包是 ad-hoc 签名，没有经过 Apple 公证。**从别的机器拷来的包，第一次打开可能会被 macOS 拦下来。** 它会说无法验证开发者、无法确认是否包含恶意软件，按钮通常只有「移到废纸篓」和「完成」，没有「仍要打开」。
...
（Releases 里的正式包已走 Developer ID 签名加公证，上面这一整节用不上；这里保留给本地 ad-hoc 构建的场景。）
...
会自己回来。第一次从「应用程序」启动时，哨兵自己把 LaunchAgent 配好（等价于以前安装器干的事），之后每次开机它自动出现在菜单栏。macOS 会弹一条「已添加登录项」的通知，这是正常的，不用管。

不想让它开机自启，在设置里关掉，下次开机生效；当前这次照常运行。

### 设置

面板底部那排图标里有个齿轮，点开是一个独立的设置窗口，分三组：

- **通知** —— 总开关「通知我」。下面可以分别开任务干完、任务出问题、通道熔断或余额不够；「怎么说」选每条都弹，或攒 1 / 5 分钟合成一条。总开关关掉时下面整组置灰。
- **历史** —— 最多留多少条。默认 500。超出的旧记录会自动清掉，正在跑的线不会被清。
- **启动与目录** —— 开机时自动启动（关掉后下次开机生效）；盯这个文件夹。监视目录如果启动时用环境变量指定过，会置灰并写着由启动配置指定。
