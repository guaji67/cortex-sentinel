...
**拖进「应用程序」这条路不带开机自启。** 重启电脑后菜单栏不会自己出现哨兵，得再手动打开一次。

开机自启来自源码那条路的安装脚本写的一份 launchd 配置。所以现在要拿到自启，机器上得有 Xcode 命令行工具（安装脚本要用 `codesign` 和 `lipo` 核一遍包）。一台完全没装过开发工具的电脑，眼下只能手动开。
