...
async function rulesPage(){
  ...
  try{
    ...
    const peers=aiState.peers||[], machines=[{id:'local',name:'本机',installed:aiState.installed,at:new Date().toISOString()},...peers.map(p=>({id:p.id,name:p.catalog?.machine||p.id,installed:p.catalog?.installed||[],at:p.last_success,error:p.error}))];
    const packages=new Map();
    for(const p of aiState.packages||[])packages.set('local/'+p.id,{...p,source:'local'});
    for(const peer of peers)for(const p of peer.catalog?.packages||[])packages.set(peer.id+'/'+p.id,{...p,source:peer.id});
    const cells=p=>machines.map(m=>{const installed=m.installed.find(i=>i.id===p.id);const same=installed?.source===(m.id==='local'?p.source:(p.source===m.id?'local':null));
      return `<td>${installed?`<span class="rule-state ${installed.state==='conflict'?'conflict':installed.digest===p.digest?'installed':'pending'}">${esc(installed.state==='conflict'?'本机差异':installed.digest===p.digest?'版本一致':'版本不同')}</span><small>${esc(installed.activation||'尚无调用证据')}</small>`:'<span class="subtle">未登记安装</span>'}${m.error||age(m.at)>420?'<small class="rule-warning">旧读数 · 不代表当前状态</small>':''}</td>`;}).join('');
    $('#main').innerHTML=head('AI 规则','正本 → 安装 → 调用','按包管理，不绑定机器分工。版本一致不等于 AI 已经使用。','<button id="ai-sync">刷新局域网</button>')+
      ...
      `<div class="table-scroll"><table class="ticket-table rules-table"><thead><tr><th>规则 / 正本</th>${machines.map(m=>`<th>${esc(m.name)}<small>${esc(time(m.at))}</small></th>`).join('')}<th>本机操作</th></tr></thead><tbody>${[...packages.values()].map(p=>`<tr><td><span class="rule-kind ${p.kind}">${esc(aiLabels[p.kind]||'未知')}</span><b>${esc(p.title||p.id)}</b><small>${esc(p.id)} · 来源 ${esc(p.source==='local'?'本机':p.source)}</small>${p.error?`<small class="rule-warning">${esc(p.error)}</small>`:''}</td>${cells(p)}<td><button data-rule-source="${esc(p.source)}" data-rule-id="${esc(p.id)}" ${p.error?'disabled':''}>查看 / 安装</button></td></tr>`).join('')||'<tr><td colspan="4">尚未登记规则。下面可以登记现有 Skill，正文和目录结构保持不变。</td></tr>'}</tbody></table></div>`+
...
async function previewRule(source,id){
  try{
    ...
    show(p.title||id,`${isHook?'Hook':'Skill'} · ${source} · ${result.digest.slice(0,12)}`,
      `<p>${esc(result.warning)}</p>${isHook?`<p>触发事件：<b>${esc(p.hook.event)}</b> · ${esc(p.hook.interpreter)} · ${esc(p.hook.entry)}</p>`:''}`+
