...
final class SentinelStore {
    ...
    private func refreshRoutePreview() {
        ...
        Task { @MainActor [weak self] in
            let outcome = await Task.detached(priority: .utility) {
                await CortexRoutePreviewFetcher.fetch(
                    environment: environment,
                    watchDirectory: watchDirectory,
                    fallbackRepositoryRoot: fallbackRepositoryRoot,
                    homeDirectory: homeDirectory,
            ...
            case let .failure(reason):
                self.routePreview = CortexRoutePreviewDisplayState(
                    payload: self.routePreview?.payload,
    ...
    private func refreshTelemetrySummary() {
        guard !telemetrySummaryFetchInFlight else {
            return
        }
        telemetrySummaryFetchInFlight = true
        let environment = self.environment
        let watchDirectory = self.paths.logsDirectory
        let fallbackRepositoryRoot = self.paths.repositoryRoot
        let homeDirectory = FileManager.default.homeDirectoryForCurrentUser.path
        Task { @MainActor [weak self] in
            let outcome = await Task.detached(priority: .utility) {
                await CortexTelemetrySummaryFetcher.fetch(
                    environment: environment,
                    watchDirectory: watchDirectory,
                    fallbackRepositoryRoot: fallbackRepositoryRoot,
                    homeDirectory: homeDirectory,
            ...
            self.telemetrySummaryFetchInFlight = false
            switch outcome {
            case let .success(payload):
            ...
            case let .failure(reason):
                self.telemetrySummary = CortexTelemetrySummaryDisplayState(
                    payload: self.telemetrySummary?.payload,
                    fetchedAt: self.telemetrySummary?.fetchedAt,
                    failureText: reason,
                    failureAt: self.now()
                )
            }
