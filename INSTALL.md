...
面板左下角有一行显示自启状态：写着「已开机自启」就是开着的；写着「开机自启」并且旁边有个开关，说明还没开，拨一下就行。

不想让它开机自启，去「系统设置」→「通用」→「登录项与扩展」里关掉。**关掉之后要开回来也在同一个地方**——哨兵不会自作主张把你关掉的东西开回去。

（面板上那一行如果写的是「自启由系统托管」，说明这台机器是用下面那条路装的，自启归 launchd 管，不走登录项，也就没有开关。）
