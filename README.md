...
- **余额**：
  - 额度行数字看额度，行下方一条细横条看时间（当前 5 小时窗的剩余时间占比，见底即将重置）；鼠标悬停 0.5 秒出详细数据（用量、重置时间、累计消费）
  - **Command Code**：每把 key 一行，显 5 小时 / 周滚动窗剩余百分比和月度 credits 余额（月 $xx.xx）。key 自动识别官方 CLI 登录态（`cmd login` 写下的 `~/.commandcode/auth.json`）和环境变量；识别不到（比如只办了 API key 没用 CLI）时，余额区给「点击填写 API Key」引导，点了就地弹填写面板，设置窗里也能增删。只查账务接口，不消耗 credits。面板里直接点行名字就能改名（GLM 行同样支持）。
...
有用，但只在按它的目录约定提供数据时，派工线才会出现。它不绑定某个叫 Cortex 的代码仓库，监视目录可以是任意文件夹。没有那些文件时应用照常启动，面板会写明监视目录不存在，并指出该设哪个环境变量。Input 探针只要网络通就能用；Command Code、GLM、Cursor、GPT 官方、AIO 各自取决于本机有没有对应的登录态或 key，互相独立，缺哪个少哪一块。
